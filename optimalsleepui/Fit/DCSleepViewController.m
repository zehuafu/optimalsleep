#import "DCSleepViewController.h"
    ...
    [self.healthStore saveObject:stepCorrelationItem withCompletion:^(BOOL success, NSError *error) {
        dispatch_async(dispatch_get_main_queue(), ^{
            if (success) {
                ...
                UIAlertView *doneAlertView = [[UIAlertView alloc] initWithTitle:@"Reminder" message:@"Added Successfully" delegate:nil cancelButtonTitle:@"OK!" otherButtonTitles:nil, nil];
                [doneAlertView show];
            }
            else {
                NSLog(@"The error was: %@.", error);
                UIAlertView *doneAlertView = [[UIAlertView alloc] initWithTitle:@"Reminder" message:@"Error" delegate:nil cancelButtonTitle:@"OK!" otherButtonTitles:nil, nil];
                [doneAlertView show];
                return ;
            }
        });
...
- (HKQuantitySample *)stepCorrelationWithStepNum:(double)stepNum {
    ...
    HKDevice *device = [[HKDevice alloc] initWithName:@"iPhone" manufacturer:@"Apple" model:@"iPhone" hardwareVersion:@"" firmwareVersion:@"" softwareVersion:@"" localIdentifier:@"" UDIDeviceIdentifier:@""];
